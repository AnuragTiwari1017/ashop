...
from .models import *
from ashop.settings import RAZORPAY_API_KEY,RAZORPAY_API_SECRET_KEY
import razorpay
...
def shop(Request, mc, sc, br):
    if (mc == 'All' and sc == 'All' and br == 'All'):
        ...
        data = Product.objects.filter(maincategory=Maincategory.objects.get(name=mc), subcategory=Subcategory.objects.get(
            name=sc), brand=Brand.objects.get(name=br)).order_by('id').reverse()
    ...
    if (Request.method == "POST"):
        ...
        if (p == cp):
            ...
            if (user):
                ...
                user.save()
                b.save()
                subject = 'You Account is Created:Team Ashop'
...
def searchPage(Request):
    ...
    data=Product.objects.filter(Q(name__icontains=search)|Q(color__icontains=search)|Q(size__icontains=search)|Q(description__icontains=search)|Q(stock__icontains=search))
    maincategory = Maincategory.objects.all()
    subcategory = Subcategory.objects.all()
    brand = Brand.objects.all()
    ...
    if(Request.method=="POST"):
        ...
        try:
            ...
            if(user.is_superuser):
                ...
                buyer.otp=otp
                buyer.save()
                subject = 'OTP for Password Reset:Team Ashop'
